...
async function loadData(uid, ufirstName, ulastName, uusername, umobile, ubirthDate, ucountry, ucity, ustreet,
                        uhouseNumber, upostalCode){
    ...
    if(secondInNav.value == 'Accounts')
    {
        secondInNav.setAttribute('onclick',`fetchUserAccounts(${uid})`);
        thirdInNav.setAttribute('onclick',`fetchUserTransactions(${uid})`);
    }
    ...
    // }
    MyPage(ufirstName, ulastName, uusername, umobile, ubirthDate, ucountry, ucity, ustreet,
        uhouseNumber, upostalCode);
}
function MyPage(ufirstName, ulastName, uusername, umobile, ubirthDate, ucountry, ucity, ustreet,
                uhouseNumber, upostalCode) {
    ...
    fullName.innerHTML = ufirstName +' '+ ulastName;
    let email = document.createElement('h5');
    email.innerHTML = 'Username: ' + uusername;
    let mobile = document.createElement('h5');
    mobile.innerHTML = 'Mobile: ' + umobile;
    let birthDate = document.createElement('h5');
    birthDate.innerHTML = 'Birth Date: ' + ubirthDate;
    let address = document.createElement('h5');
    address.setAttribute('style','white-space:pre-line');
    address.innerHTML = ucountry + ',' + ucity + '\n' + ustreet + ',' + uhouseNumber + '\n' +'Postal Code: ' +upostalCode;
...
async function fetchUserAccounts(user_id){
    ...
    crAccBt.setAttribute('onclick',"location.href='../account/create'");
    mainDiv.appendChild(crAccBt);
    let balances;
    //const balanceUrl = `http://localhost:8080/transaction/balance?id=${user_id}`;
    const url = `http://localhost:8080/account/userAll?id=${user_id}`;
    //let token = localStorage.getItem("token");
